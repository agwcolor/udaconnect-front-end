...
class Connection extends Component {
  ...
  getConnections = (personId) => {
    if (personId) {
      ...
      // use localhost:5002 running locally, not in a container
      // use localhost:30003 to connect to kubernetes from outside
      // use 10.42.0.34:5005  (service ip + grpc server port - not actually implemented here from front end. Please use writer.py to test
      fetch(
        `http://10.42.0.34:5005/api/persons/${personId}/connection?start_date=2020-01-01&end_date=2020-12-30&distance=5`
      )
